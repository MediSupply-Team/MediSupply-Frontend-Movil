...
async function testHealthEndpoint(name, url) {
  try {
    ...
    const response = await axios.get(`${url}/health`, { 
      timeout: 5000,
      headers: { 'User-Agent': 'MediSupply-Clientes-Test/1.0' }
    });
...
async function testSpecificEndpoints(env) {
  console.log(`\n🎯 Testing endpoints específicos de la app CLIENTES...`);
  const config = ENVIRONMENTS[env];
  
  if (env === 'aws') {
    // Test BFF Cliente específicamente
    await testHealthEndpoint('BFF Cliente', config.bffCliente);
    
    // Test endpoints específicos del BFF Cliente
    try {
      const ordersUrl = `${config.bffCliente}/api/v1/orders`;
      console.log(`🔍 Testing orders endpoint: ${ordersUrl}`);
      
      // Solo un GET para verificar que el endpoint existe
    ...
    // Test catálogo a través de BFF Venta
    try {
      const catalogUrl = `${config.bffVenta}/catalog`;
      console.log(`🔍 Testing catalog (via BFF Venta): ${catalogUrl}`);
...
async function main() {
  console.log('🚀 MediSupply Clientes - Endpoint Testing');
  console.log('══════════════════════════════════════════════');
